...
type SensitiveField = "title" | "username" | "email" | "password" | "pin" | "url" | "notes";

function context(field: SensitiveField) {
  return `vault.${field}`;
}

export function encryptVaultField(field: SensitiveField, value: string | null, entryId: string) {
...
export function publicVaultEntry(entry: VaultEntry) {
  return {
    ...
    password: decrypt(entry.password, `${context("password")}:${entry.id}`),
    pin: entry.pin ? decrypt(entry.pin, `${context("pin")}:${entry.id}`) : null,
    url: entry.url ? decrypt(entry.url, `${context("url")}:${entry.id}`) : null,
    notes: entry.notes ? decrypt(entry.notes, `${context("notes")}:${entry.id}`) : null,
