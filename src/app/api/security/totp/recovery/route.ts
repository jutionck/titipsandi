...
export async function POST(req: NextRequest) {
  ...
  const policies = [
    {
      scope: "totp-recovery-regenerate-user",
  ...
  if (
    !user?.totpSecret ||
    !user.totpEnabledAt ||
    !verifyTotpCode(decryptTotpSecret(user.totpSecret, session.userId), code)
  ) {
  ...
  await recordSecurityAuditEvent({
    userId: session.userId,
    action: SECURITY_AUDIT_ACTIONS.RECOVERY_CODES_REGENERATED,
    outcome: "SUCCESS",
    actorType: "OWNER",
  });
  await clearRateLimits(policies);

  return privateJson({ success: true, recoveryCodes });
}
